Delimiter ;;
Drop procedure if exists getFilteredPeopleNearByPaginatedNew;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `getFilteredPeopleNearByPaginatedNew`(IN `usersid` INT, IN `game_id` INT, IN `minAge` INT, IN `maxAge` INT, IN `gender` VARCHAR(100), IN `instn` VARCHAR(100), IN `city` VARCHAR(100), IN `limiter` INT, IN `skip` INT, IN `timestamp` BIGINT)
BEGIN
...
inner join UserInterest_tbl utt
on ut.user_id=utt.user_id
where ",@usersid,"<>ut.user_id 
and ('",@gameid,"' like '",-1,"' or utt.gameid like '",@gameid,"')
and not exists
(
    Select 1 from User_Playmates_tbl upt
    where upt.userid=",@usersid,"
