Delimiter ;;
Drop procedure if exists getUserRequestNotification;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `getUserRequestNotification`(IN `userid` INT, IN `limiter` INT, IN `skip` INT, IN `timestamp` BIGINT)
BEGIN
...
Case when gt.gamename is null then '' else gt.gamename end as gamename,
               ...
               where ut.activityid in (1006,1008)
               order by CreatedDate desc;
