Delimiter ;;
Drop procedure if exists getFilteredPeopleNearByPaginatedPlayingThisWeek;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `getFilteredPeopleNearByPaginatedPlayingThisWeek`(IN `usersid` INT, IN `game_id` INT, IN `minAge` INT, IN `maxAge` INT, IN `gender` VARCHAR(100), IN `instn` VARCHAR(100), IN `city` VARCHAR(100), IN `limiter` INT, IN `skip` INT, IN `timestamp` BIGINT)
BEGIN
...
(
    ...
    (
        ...
        and TIMESTAMPDIFF(Day,et.Start_date,CURDATE())<7
