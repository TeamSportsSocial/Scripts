Delimiter ;;
Drop procedure if exists getFilteredPeopleNearByPaginatedOnInsertedDate;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `getFilteredPeopleNearByPaginatedOnInsertedDate`(IN `usersid` INT, IN `game_id` INT, IN `minAge` INT, IN `maxAge` INT, IN `gender` VARCHAR(100), IN `instn` VARCHAR(100), IN `city` VARCHAR(100), IN `limiter` INT, IN `skip` INT, IN `timestamp` BIGINT)
BEGIN
...
ORDER BY ut.InsertedDate limit"," ");
